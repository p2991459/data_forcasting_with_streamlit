...
def test_and_predict(df, col_name, horizon):
    ...
    train = df[col_name][:-16]
    print("control is in test_and_predict and column name is ",col_name)
    test = df[['date', col_name]][-16:]
    total_len = len(df)
...
# print(col1,col2,"col1 and col2 here")
...
if forecast_btn:
    ...
    # pred_ax.set_xticks([0, 1, 2, 3, 4, 5, 6, 7])  # Make sure the number of tick locations matches the number of labels
    # pred_fig.set_ticklabels(['Label 1', 'Label 2', 'Label 3', 'Label 4', 'Label 5', 'Label 6', 'Label 7', 'Label 8'])

...
if file:
    n = 1
    nfirstlines = []
    with io.BytesIO(file.getvalue()) as f, open("temp.csv", "w") as out:
        for x in range(n):
...
if submitted:
    ...
    # Additional processing or confirmation message can be displayed here

@st.cache_data
def read_data():
    df = pd.read_csv("temp.csv", index_col=0, dtype={'date': str, 'Audi Q3': np.float64})
    return df, df.T
